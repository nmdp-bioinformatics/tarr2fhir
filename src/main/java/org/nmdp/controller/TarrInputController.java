...
public class TarrInputController implements TarrApi {
    ...
    @ResponseBody
    public ResponseEntity<String> tarr2fhir(@ApiParam(value = "" ,required=true )  @Valid @RequestBody String xml, @RequestParam String labName, @RequestParam String reportingCenter, @RequestParam String sampleType) {
      try {
          ParseInputFiles aParser = new ParseInputFiles();
          List<String> aXmlInput = new ArrayList<>();
          aXmlInput.add(xml);
          aParser.setMyInputFiles(aXmlInput);
          aParser.setMetaData(labName, reportingCenter, sampleType);
          aParser.process();
    ...
    @ResponseBody
    public ResponseEntity<String> tarr2fhirmulti(@ApiParam(value = "The file to upload.") @Valid @RequestPart(value="upfile", required=false) MultipartFile upfile, @RequestParam String labName, @RequestParam String reportingCenter, @RequestParam String sampleType) {
        try {
